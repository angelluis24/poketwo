...
class Bot(commands.AutoShardedBot):
    ...
    async def sessions(self):
        return {
            int(x): Session(y)
            for x, y in (await self._connection._get("gateway_sessions")).items()
        }
    ...
    async def start(self):
        ...
        self._redis = await aioredis.create_redis_pool(self.config.redis_url)
    ...
    async def is_owner(self, user):
        if inspect.iscoroutinefunction(user):
            user = await user()
