...
class State:
    ...
    async def _private_channels(self):
        ...
        return [
            DMChannel(me=self.user, state=self, data=x)
            for x in results
            if not x["guild_id"]
        ]
    ...
    async def _get_private_channel_by_user(self, user_id):
        return utils.find(
            lambda x: x.recipient.id == user_id, await self.private_channels()
        )
    ...
    async def parse_message_delete_bulk(self, data, old):
        ...
        if old:
            ...
            for old_message in old:
                ...
                if channel:
                    messages.append(
                        self.create_message(channel=channel, data=old_message)
                    )
    ...
    async def parse_guild_emojis_update(self, data, old):
        ...
        if guild:
            ...
            after_emojis = tuple(
                map(lambda x: self.store_emoji(guild, x), data["emojis"])
            )
    ...
    async def get_channel(self, channel_id):
        if not channel_id:
            return None
        return await self._get_private_channel(
            channel_id
        ) or await self._get_guild_channel(channel_id)
