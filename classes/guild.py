"""
This file is adapted from twilight-dispatch, which is released under the ISC license.
See https://github.com/chamburr/twilight-dispatch for full license details.
"""
...
class Guild(guild.Guild):
    ...
    async def _channels(self):
        channels = []
        for channel in await self._state._members_get_all(
            "guild", key_id=self.id, name="channel"
        ):
    ...
    async def _members(self):
        members = []
        for member in await self._state._members_get_all(
    ...
    async def _roles(self):
        roles = []
        for role in await self._state._members_get_all(
            "guild", key_id=self.id, name="role"
        ):
    ...
    async def _voice_states(self):
        voices = []
        for voice in await self._state._members_get_all(
            "guild", key_id=self.id, name="voice"
        ):
            if voice["channel_id"]:
